from app.core import celery_app as _celery_app  # noqa: F401  # 确保 shared_task 使用同一个 Celery app
...
def test_tick_product_full_sync_triggers_and_updates_last_run():
    """
    集成校验：直接触发 tick_product_full_sync，依赖外部已经准备好的 schedules 数据。
    仅拦截 sync_start_full.delay 以避免真实串起 Celery/Shopify。
    """
    ...
    # monkeypatch.setattr(scheduler_tick.sync_start_full, "delay", _fake_delay)

    result = scheduler_tick.tick_product_full_sync.run()

    assert isinstance(result, dict)
    # 如果命中了触发窗口，则应调用 sync_start_full；否则允许返回非 fired 状态
